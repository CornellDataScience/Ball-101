...
def run(
    ...
    ret=False,  # return values as a list of tuples
    ...
    while True:
        try:
            ...
            # if frame_idx % 1000 == 0:
            #     # gc.collect()
            #     print("f:", frame_idx, gc.get_count())

            # Inference
            t2 = time_sync()
            result = next(results)
            path, im0 = result.path, result.orig_img
            t3 = time_sync()
            dt[1] += t3 - t2
            ...
            LOGGER.info(
                f"Done. YOLO:({t3 - t2:.3f}s), StrongSORT:({t5 - t4:.3f}s)"
            )
