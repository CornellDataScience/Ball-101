...
if __name__ == "__main__":
    if len(sys.argv) < 5:
        print("Usage: python track_v7.py source yolo_weights classes, save_vid")
    else:
        source = (sys.argv[1],)
        yolo_weights = (sys.argv[2],)
        classes = (json.load(sys.argv[3]),)
        save_vid = (sys.argv[4],)
    if save_vid == "True":
        save_vid = True
    else:
    ...
    parser.add_argument(
        "--weights", nargs="+", type=str, default=yolo_weights, help="model.pt path(s)"
    )
    parser.add_argument(
        "--source", type=str, default=source, help="source"
