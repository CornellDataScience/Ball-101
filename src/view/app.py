...
def process_results():
    ...
    team_df = pd.DataFrame({
        'Team 1': [result_dict['Team 1'], result_dict['Team 1 Score'], 
                   result_dict['Team 1 Possession']],
        'Team 2': [result_dict['Team 2'], result_dict['Team 2 Score'], 
    ...
    players_dict = {key: value for key, value in result_dict.items()
                    if key not in general_df.columns and key not in team_df.columns and
                    key not in coordinates_df.columns}
    # Remove team score and possession details from players dictionary
    team_keys = ['Team 1 Score', 'Team 2 Score', 'Team 1 Possession', 'Team 2 Possession']
    players_dict = {key: value for key, value in players_dict.items() if key not in team_keys}
